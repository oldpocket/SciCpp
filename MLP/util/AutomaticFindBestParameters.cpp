...
// find the best parameters to run a NPL
int automaticFindBestParameters()
{
	...
	split(data_set_manager->getDataset(), d_train, d_valid, 30);

	int n_trials = 3;

	// each number is the possible number of neurons in the hidden layer
	int hidden_unit[] = {5, 6, 7, 8, 9}; // num of hidden unit, set the values of search
	// the code bellow will test each one of options above in hidden_unit[]
	// set true for hidden unit estimation
	if (true)
	{
		...
		for (int hu = 0; hu < 5; hu = hu + 1)
		{
			...
			{ // 3 trials on the same data, for local minima
			...
			if (valid_error < best_err * 0.99)
			{ // min increment to increase complexity
	...
	if (true)
	{
		...
		while (data_set_manager->hasNextFold())
		{
			...
			net1->train(data_set_manager->getTrainingSet(), best_eta, best_alpha, best_lambda, epoch_limit, false /*printout*/, true /*mee*/);
			double e = net1->test(data_set_manager->getValidationSet(), false, true); // true for mee, false for mse
			cout << "fold " << data_set_manager->getActualFoldNum() << " loss(mee) " << e << endl;
			test_error += e;
		}
	...
	const vector<vector<double>> &d_test = test_set_manager->getDataset();
