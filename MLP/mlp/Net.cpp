...
// "t" is the topology in the string format:  "num of unit per layer", separated by a space
Net::Net(string t, bool classification, unsigned first_target_col, double *scale, double *translate) : 
Net::Net(getTopology(t), classification, first_target_col, scale, translate)
{	
}
...
void Net::getOutputs(vector<double> &output_vals) const
{
	...
	{
		output_vals.push_back(layers_.back()[n].getOutputVal());
				  ...
				  unsigned epoch_max, bool printout, bool mee)
{
	...
	while ((epoch < epoch_max) && (!done_looping))
	{
		...
		if ((epoch % validation_frequency) == 0)
		{
			...
			if (printout)
				errFile << epoch << ", " << t_error << ", " << this_validation_loss << endl;
...
double Net::test(const vector<vector<double>> d_test, bool print_output, bool mee)
{
	...
	for (unsigned i_sample = 0; i_sample < set_size; i_sample++)
	{
		...
		if (classification_)
		{ // check accuracy, 1 if >=0.5; 0 otherwise
			for (unsigned j = 0; j < target_vals.size(); j++)
			{
				if (abs(target_vals[j] - output_vals[j]) < 0.5)
					right = right + 1.0;
				else
					wrong += 1.0;
			}
	...
	if (classification_)
	{
		...
		if (print_output)
			cout << "test accuracy " << accuracy << endl; // print the accuracy
	}
...
void Net::resetNetWeights(void)
{
	...
	for (unsigned layer = 0; layer < num_layers; ++layer)
	{
		...
		{
			layers_[layer][neuron].resetOutputWeights();
		}
	}
}
...
void Net::setNetWeights(const vector<vector<Weights>> w)
{
	...
	for (unsigned layer = 0; layer < num_layers; ++layer)
	{
		...
		{
			layers_[layer][neuron_num].setOutputWeights(w[layer][neuron_num]);
		}
	}
}
...
vector<vector<Weights>> Net::getNetWeights()
{
	...
	for (unsigned layer_index = 0; layer_index < num_of_layers; ++layer_index)
	{
		...
		{
			neuron_weights = layers_[layer_index][neuron_n].getOutputWeights();
...
vector<unsigned> Net::getTopology(string t)
{
